...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('json_path', type=str, help='Путь к JSON-файлу с рецептами')
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        if image_path and not image_path.exists():
            self.stderr.write(self.style.ERROR(f"Файл изображения {image_path} не найден"))
            return
        ...
        ingredient_ids = list(
            Ingredient.objects.filter(id__range=(1, 2000)).values_list('id', flat=True)
        )
        if not ingredient_ids:
            self.stderr.write(self.style.ERROR("Не найдены ингредиенты с ID 1–2000."))
            return
        ...
        for data in recipes:
            try:
                ...
                self.stderr.write(self.style.ERROR(f"Автор {data['author']} не найден"))
                continue

            recipe = Recipe.objects.create(
                name=data["name"],
                author=author,
                text=data.get("description", "Автоматически сгенерированное описание."),
            ...
            if image_path:
                with image_path.open('rb') as img_file:
                    recipe.image.save(image_path.name, ImageFile(img_file), save=True)
            else:
                # Если не указан путь к картинке — можно скипнуть или задать через FileField с заглушкой
                self.stderr.write(f"⛔ Нет изображения для рецепта {recipe.name}, добавьте через --image")

            # Добавим 3–6 случайных ингредиентов
            selected_ids = random.sample(ingredient_ids, k=random.randint(3, 6))
